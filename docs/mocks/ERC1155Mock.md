...
```solidity
function transferBatch(address from, address to, uint256[] ids, uint256[] values) external nonpayable
```
...
```solidity
function transferSingle(address from, address to, uint256 id, uint256 value) external nonpayable
```
