...
contract depositETHAsset is YieldBoxUnitConcreteTest {
    ...
    function test_depositETHAssetRevertWhen_AssetIsNotERC20(uint64 depositAmount) public {
        ...
        // Try to deposit an incorrect asset
        vm.expectRevert(InvalidTokenType.selector);
        yieldBox.depositETHAsset{value: depositAmount}(erc721AssetId, users.alice, depositAmount);
    }
    ...
    function test_depositETHAssetRevertWhen_AssetIsNotWrappedNative(uint64 depositAmount) public {
        ...
        yieldBox.depositETHAsset{value: depositAmount}(DAI_ASSET_ID, users.alice, depositAmount);
    }
    ...
    function test_depositETHAssetRevertWhen_AmountIsGreaterThanValue() public {
        ...
        yieldBox.depositETHAsset{value: WEI_AMOUNT}(WRAPPED_NATIVE_ASSET_ID, users.alice, MEDIUM_AMOUNT);
    }
    ...
    function test_depositETHAsset_AssetIsSupported(uint64 depositAmount) public {
        (uint256 totalShare, uint256 totalAmount) = yieldBox.assetTotals(WRAPPED_NATIVE_ASSET_ID);
        ...
        vm.expectEmit();
        emit TransferSingle(users.alice, address(0), users.alice, WRAPPED_NATIVE_ASSET_ID, expectedShare);

        // It should emit a `Deposited` event
        vm.expectEmit();
        emit Deposited(
            users.alice, users.alice, users.alice, WRAPPED_NATIVE_ASSET_ID, depositAmount, expectedShare, 0, 0, false
        );

        yieldBox.depositETHAsset{value: depositAmount}(WRAPPED_NATIVE_ASSET_ID, users.alice, depositAmount);
    ...
    function test_depositETHAssetRevertWhen_ValueRefundFails(uint64 depositAmount, uint64 specifiedAmount) public {
        ...
        yieldBox.depositETHAsset{value: depositAmount}(WRAPPED_NATIVE_ASSET_ID, users.alice, WEI_AMOUNT);
    }
    ...
    function test_depositETHAsset_ExcessOfValueIsRefunded(uint64 depositAmount) public {
        ...
        yieldBox.depositETHAsset{value: uint256(depositAmount) + 1}(WRAPPED_NATIVE_ASSET_ID, users.alice, depositAmount);
